"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.isEligible = exports.debugStructuredSemantic = exports.compareCefr = exports.calculateMatchScore = exports.calculateConstraintScore = exports.MATCHING_WEIGHTS_HISTORY = exports.MATCHING_ALGORITHM_VERSION = void 0;
...
var _vectorMath = await jitiImport("./vectorMath");
var _synonyms = await jitiImport("./synonyms");
var _universityTiers = await jitiImport("./universityTiers");
var _companyPrestige = await jitiImport("./companyPrestige");
var _education = await jitiImport("./education");const MATCHING_ALGORITHM_VERSION = exports.MATCHING_ALGORITHM_VERSION = 'v1.1.0'; // Audit trail: maps version → weights snapshot + activation date
const MATCHING_WEIGHTS_HISTORY = exports.MATCHING_WEIGHTS_HISTORY = { 'v1.0.0': { activatedAt: '2025-01-01', weights: { semantic: 0.40, hard: 0.20, constraint: 0.00, industry: 0.10, experience: 0.10, education: 0.10, careerPrestige: 0.10 } }, 'v1.1.0': { activatedAt: '2026-05-18', weights: { semantic: 0.50, hard: 0.30, constraint: 0.00, industry: 0.05, experience: 0.00, education: 0.10, careerPrestige: 0.05 } } };
/**
...
const normalizeSkillName = (value) => (value || '').toLowerCase().trim();
...
exports.compareCefr = compareCefr;
const getDegreeOrdinal = (degreeLevel) => (0, _education.getEducationLevelOrdinal)(degreeLevel);
 ...
 */
const areSkillsSimilar = (s1, s2) => {
  const a = s1.toLowerCase().trim();
  const b = s2.toLowerCase().trim();
  if (a === b) return true;

  // Synonym dictionary check
  if ((0, _synonyms.areSynonyms)(a, b)) return true;
  ...
  if ((0, _synonyms.areFuzzyMatch)(a, b)) return true;
...
const FAMILY_GROUP_COMPATIBILITY = {
...
const matchesAnyPattern = (texts, patterns) =>
patterns.some((pattern) => texts.some((text) => textContainsTerm(text, pattern)));

const getMatchingBackgroundFamilyRules = (
texts,
patternKey) =>
BACKGROUND_FAMILY_RULES.filter((rule) => matchesAnyPattern(texts, rule[patternKey]));

const getFamilyGroups = (rules) => Array.from(new Set(rules.map((rule) => rule.familyGroup)));
...
const termRelationCache = new Map();

const computeTermRelationScore = (left, right) => {
  if (left === right) return 1;
  if (areSkillsSimilar(left, right)) return 1;

  const relation = (0, _synonyms.getSkillRelationScore)(left, right);
...
const getTermRelationScore = (a, b) => {
 ...
 */
const sigmoidNormalizeCosine = (cosine) => {
  const CENTER = 0.65;
  const STEEPNESS = 10;
  const FLOOR_COS = 0.30;
  const CEIL_COS = 0.97;
  const sigmoid = (x) => 1 / (1 + Math.exp(-STEEPNESS * (x - CENTER)));
...
const collectCandidateEvidenceTexts = (candidate) => compactStrings([
...
const collectCandidateRoleTerms = (candidate) => compactStrings([
candidate.current_job_function,
...(candidate.target_job_functions || []),
...
const collectJobSemanticTerms = (job) => compactStrings([
job.title,
job.job_function,
...
const mergeSkillEvidence = (target, skill) => {
...
const buildCandidateSkillPool = (candidate) => {
...
const getBestPairwiseRelation = (leftTerms, rightTerms) => {
  let best = 0;
...
const getSemanticFamilySupport = (candidateTexts, jobTexts) => {
  let best = 0;
...
const getEducationTrajectorySupport = (candidate, jobSemanticTerms) => {
...
const getExperienceTrajectorySupport = (candidate, jobSemanticTerms) => {
...
const getProvenRoleExperienceAlignment = (job, candidate) => {
...
const hasExplicitHardSkillInventory = (candidateSkillPool) =>
...
};exports.debugStructuredSemantic = debugStructuredSemantic;
...
candidateSkillPool) =>
{
  ...
  const addHeadroomScaledBonus = (bonus) => {
 ...
 */
const isEligible = (job, candidate) => {
...
const calculateConstraintScore = (job, candidate) => {
  const { constraints } = job;
  ...
  if (typeof noticeMonths === 'number') {
    if (noticeMonths <= 1) availabilityScore = 1.0;else
    if (noticeMonths <= 2) availabilityScore = 0.85;else
    if (noticeMonths <= 3) availabilityScore = 0.7;else
 ...
 */exports.calculateConstraintScore = calculateConstraintScore;
const calculateEducationScore = (job, candidate) => {
  ...
  map((e) => ({
    ...
    uniTier: (0, _universityTiers.getUniversityTier)(e.institution)
  })).
  sort((a, b) => {
  ...
  const uniScore = (0, _universityTiers.universityTierToScore)(best.uniTier);
 ...
 */
const calculateCareerPrestigeScore = (candidate) => {
  ...
  const recentTier = (0, _companyPrestige.getCompanyTier)(recentCompany.company);
  const recentScore = (0, _companyPrestige.companyTierToScore)(recentTier);

  if (sorted.length === 1) return recentScore;

  // Average score of other companies
  const otherScores = sorted.slice(1).map((exp) => (0, _companyPrestige.companyTierToScore)((0, _companyPrestige.getCompanyTier)(exp.company)));
...
const getLevelMatchScore = (candidateLevel, requiredLevel) => {
...
const evaluateHardSkills = (job, candidate, candidateSkillPool) => {
  ...
  const explicitCandClusters = new Set();
  for (const cs of candSkills) {
    if (cs.level_source === 'inferred') continue;
    const idx = (0, _synonyms.getSkillClusterIndex)(cs.skill_name);
  ...
  const scoreBucket = (bucket) => {
    if (bucket.length === 0) return { score: null, coverage: null };

    // Compute domain proximity for THIS bucket:
    // fraction of the bucket's distinct clusters that the candidate touches.
    const bucketClusters = new Set();
    for (const js of bucket) {
      const idx = (0, _synonyms.getSkillClusterIndex)(js.skill_name);
    ...
    for (const js of bucket) {
      ...
      for (const cs of candSkills) {
        const relation = (0, _synonyms.getSkillRelationScore)(cs.skill_name, js.skill_name);
        if (relation === 0) continue;
...
const calculateMatchScore = (job, candidate) => {
  ...
  if (hasEmbedding) {
    rawEmbeddingCosine = (0, _vectorMath.cosineSimilarity)(job.embedding_vector, candidate.embedding_vector);
...
};exports.calculateMatchScore = calculateMatchScore; /* v9-98daab88bfb5097d */
