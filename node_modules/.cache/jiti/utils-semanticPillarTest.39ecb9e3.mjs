 ...
 */ // ─── Test Helpers ────────────────────────────────────────────────────────────
let passed = 0;let failed = 0;const failures = [];function fmt(score) {return `${(score * 100).toFixed(1)}%`;}function assert(condition, msg, details) {if (condition) {
    console.log(`  ✅ ${msg}`);
    passed++;
  } else {
...
function assertOrdered(scores, minGap = 0.005) {
...
function printRanking(title, items) {
  console.log(`\n  ${title}`);
...
function semanticOf(job, cand) {
  return (0, _matchingUtils.calculateMatchScore)(job, cand).semanticScore;
}
...
const FRONTEND_SENIOR_JOB = mkJob({
  id: 'job_fe_senior',
...
const DATA_SCIENTIST_JOB = mkJob({
  id: 'job_ds',
...
function testEmbeddingSensitivity() {
  ...
  const make = (vec) => ({ ...frontendStar, embedding_vector: vec });
...
function dumpDebug() {
  ...
  for (const [label, job, cand] of pairs) {
    const d = (0, _matchingUtils.debugStructuredSemantic)(job, cand);
    const sem = (0, _matchingUtils.calculateMatchScore)(job, cand).semanticScore;
